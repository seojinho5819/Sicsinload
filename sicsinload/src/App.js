...
const App = () => {
	...
	const registHandler = useCallback(
		(focusingStoreArg) => {
			if (!focusingStoreArg.id) {
				...
				const storeReviews = reviews.filter((item) => item.id === focusingStoreArg.id);
				const ratingAverage = storeReviews?.length > 0 ? storeReviews.reduce((sum, curr) => sum + curr.rating, 0) / storeReviews.length : 0;
	...
	const standbyMap = useCallback(() => {
		...
		// 내 위치 버튼
		const myLocationBtnHtml = '<button type="button" style="margin-left:5px;"><span>내위치</span></button>',
		...
		naver.maps.Event.once(map, 'init', () => {
			...
			//현재위치에서 주변 음식점 검색
			naver.maps.Event.addDOMListener(customControl2.getElement(), 'click', () => {
				getAxios(mylocation, map.getBounds()._max, map.getBounds()._min, map);
			});
			//검색 기반
			naver.maps.Event.addDOMListener(customControl3.getElement(), 'click', () => {
				getAxios(mylocation, map.getBounds()._max, map.getBounds()._min, map);
			});
		});
	...
	const getAroundStoreList = useCallback((bounds, savedStores, allReviews) => {
		const aroundStoreList = savedStores
			.filter((item) => item.x <= bounds._max.x && item.x >= bounds._min.x && item.y <= bounds._max.y && item.y >= bounds._min.y)
			.map((item1) => ({
				...item1,
				reviews: allReviews.filter((item2) => item2.id === item1.id),
				ratingAverage:
					allReviews.filter((item2) => item2.id === item1.id).length > 0
						? allReviews.filter((item2) => item2.id === item1.id).reduce((sum, currValue) => sum + currValue.rating, 0) /
			...
			.then((response) => {
				...
				aroundStoreList.forEach((item) => {
					...
					naver.maps.Event.addListener(marker, 'click', function (e) {
						...
						item.ratingAverage =
							allSavedReviews.filter((review) => review.id === item.id).length > 0
								? allSavedReviews.filter((review) => review.id === item.id).reduce((sum, curr) => sum + curr.rating, 0) /
	...
	const excelDownLoadHandler = (data) => {
		...
		worker.postMessage(JSON.stringify({ action: 'excelDownload', excelDatas: cloneData }));
		worker.onmessage = (m) => {
						...
						<thead>
							<tr>
								<th colSpan="7" style={{ position: 'relative' }}>
									...
									<button style={{ position: 'absolute', right: 0 }} onClick={() => excelDownLoadHandler(aroundStore)}>
										{'엑셀 다운로드'}
									</button>
								</th>
							</tr>
						...
						<TabPanel>
							...
							<div style={{ paddingRight: 50 }}>
								<div style={{ marginTop: 50 }}>
									<Infor label={'상호명'} value={focusingStore ? focusingStore.name : ''} />
								</div>
								<div
									style={{
										marginTop: 30,
								...
								<div style={{ marginTop: 30 }}>
									<Infor label={'x좌표'} value={focusingStore ? focusingStore.x : ''} />
								</div>
								<div style={{ marginTop: 50 }}>
									<Infor label={'y좌표'} value={focusingStore ? focusingStore.y : ''} />
								</div>
								<div style={{ marginTop: 50 }}>
									<Infor label={'상세주소'} value={focusingStore ? focusingStore.address : ''} />
								</div>
								<div style={{ marginTop: 50 }}>
									<Infor label={'카테고리'} value={focusingStore.category ? focusingStore.category[0] + ' >> ' + focusingStore.category[1] : ''} />
								</div>
							</div>
						</TabPanel>
						<TabPanel>
							<Menu menuInfo={focusingStore?.menuInfo ? focusingStore?.menuInfo : ''} />
						</TabPanel>
						<TabPanel>
						...
						<TabPanel>
							...
							>
								...
								>
									...
									<Rating onClick={handleRating} ratingValue={focusingStore.rating ? focusingStore.rating : 0} /* Available Props */ />
								</div>
								<div
									style={{
										marginTop: 50,
								...
								<div style={{ marginTop: 50 }}>
									<button type="button" onClick={() => registHandler(focusingStore)}>
										등록
									</button>
								</div>
							</div>
